...
- **No length cap.** This reference doesn't fix a character limit, and no calling skill imposes one either — accept what the user gives in full, however long, and split it across multiple entries or processing passes on the agent's own side if a downstream field can't hold it in one piece. Don't ask the user to condense it first; that silently picks what mattered for them.
