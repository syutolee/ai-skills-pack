...
**本技能不需要切角／價值主張文件**（跟 `ads`／`ad-creative`／`landing-page-cro` 不同）——它處理的是「已經投出去的東西表現如何」，不直接面對策略設計。切角層的判斷（這個切角假設本身還成不成立、要不要繞回去重新設計）**不在本包任何技能的範圍內**；本技能只在判斷結果指向「不是素材問題也不是投放問題」時，指出「這可能是切角層的問題」，不越級做那個判斷。
...
| 素材、投放、到達頁都排除了，還是不行 | **這可能是切角層的問題**——如實告知使用者這超出本包戰術技能的判斷範圍，需要回到切角層重新檢視。**不要自己越級判斷切角要不要換** |
