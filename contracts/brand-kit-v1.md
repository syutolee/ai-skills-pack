...
| **Layout Preferences** | Free-text bullet list, one stated preference per line, each tagged with when it was heard (`- Prefers left-aligned logo — heard in ad-creative-pro session, 2026-08-07`). No preference heard yet → section stays empty, which is legal, not an error |
...
**Layout Preferences** is append-only: any paid-tier skill in a live conversation with the user may add one line the moment a preference is stated out loud — `kickoff-pro` and `ad-creative-pro` are the two that do this today. A skill appends its own line and never edits or removes a line another run wrote.

| Section | Who writes | When |
|---|---|---|
| Logo | Whichever paid skill first reads `assets/index.md` — currently `ad-creative-pro` | On that skill's own run; refreshed only if a better-typed logo entry later appears |
| Color Palette | Same as Logo | Same |
| Layout Preferences | Any paid-tier skill mid-conversation with the user — currently `kickoff-pro`, `ad-creative-pro` | The instant a preference is stated out loud; one line appended, existing lines untouched |
...
- Prefers left-aligned logo, not centered — heard in ad-creative-pro session, 2026-08-07
```
